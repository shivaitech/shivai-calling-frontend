...
export function TextAnimations({
  ...
  // Check if current text is Arabic for special positioning
  const isCurrentTextArabic = /[\u0600-\u06FF]/.test(displayText);

  return (
    <div className={cn("relative flex items-center", containerHeight)}>
      <AnimatePresence mode="wait">
        <div key={currentLanguageIndex} className={cn("flex items-center", isCurrentTextArabic && "-ml-2")}>
          <StaggeredFade
            text={displayText}
            className={cn(
