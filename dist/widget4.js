(function () {
    ...
    try {
      ...
      if (window.SHIVAI_WIDGET_CONFIG && typeof window.SHIVAI_WIDGET_CONFIG === 'object') {
        ...
        if (widget._agent_name) {
          ...
          _wlog("🤖 Using agent name from API:", agentName);
        } else if (widget.ai_employee_name) {
          agentName = String(widget.ai_employee_name).trim();
          _wlog("🤖 Using ai_employee_name from API:", agentName);
        }
      ...
      if (!agentName && window.SHIVAI_CONFIG && typeof window.SHIVAI_CONFIG === 'object') {
        ...
        if (config.content) {
          if (config.content.companyName) {
            agentName = config.content.companyName;
            _wlog("🤖 Using companyName from SHIVAI_CONFIG as agent name:", agentName);
          }
  ...
  // ── Route guard ───────────────────────────────────────────────────────────
  // widget4.js is the CLIENT embed script. On external client websites it must
  // run everywhere (the visibility/allowed-domains check below decides). On the
  // ShivAI app host itself it is restricted to the public landing page ("/",
  // "/landing") and must stay off dashboard/auth/test routes.
  ...
  function refreshWidgetContent() {
    ...
    if (callInfoName) {
      ...
      _wlog("✅ Updated call view agent name to:", companyInfo.agentName || companyInfo.name);
    }
    ...
    if (landingAgentName) {
      ...
      _wlog("✅ Updated landing company name to:", companyInfo.name);
    }
    ...
    if (triggerAvatarWrap) {
      const triggerAvatarSrc = companyInfo.triggerButtonImage || companyInfo.logo || "";
      const initial = triggerDisplayName.trim().charAt(0).toUpperCase();
      if (triggerAvatarSrc) {
        triggerAvatarWrap.innerHTML = `<img class="shivai-trigger-avatar" src="${triggerAvatarSrc}" alt="${triggerDisplayName}" />`;
      } else {
  ...
  function startNetworkMonitor() {
    ...
    // Initial read only when widget is first opened (called from openWidget)
    measureAndUpdateNetwork();
    if (networkMonitorTimer) clearInterval(networkMonitorTimer);
    // Poll every 30s while widget is open; cleared on closeWidget
  ...
  function addWidgetStyles() {
    ...
    const companyInfo = getCompanyInfo();
    // Fallback matches the app's "Original Default" preset (grey), so an
    // unconfigured widget doesn't render the old hardcoded blue.
    ...
    const styles = `
      ...
      .shivai-trigger:hover:not(.dragging) .shivai-trigger-call-btn {
        ...
        box-shadow:
          0 6px 18px -4px ${theme.primaryColor}80,
          0 0 0 1px ${theme.primaryColor}88;
      ...
      .shivai-collapse-handle {
        ...
        box-shadow: 0 8px 22px -6px ${theme.primaryColor}80, 0 0 0 1px rgba(255,255,255,0.12) inset;
      ...
      .shivai-collapse-handle:hover {
        ...
        box-shadow: 0 10px 26px -6px ${theme.primaryColor}99, 0 0 0 1px rgba(255,255,255,0.18) inset;
      }
      ...
      /* Start-call button (landing) — themed from the selected preset */
      .start-call-btn {
      width: 100%;
      padding: 12px 24px;
      ...
      .call-view .end-call-btn.control-btn-icon {
        ...
        justify-content: center !important;
        box-shadow:
          0 10px 26px -6px ${theme.accentColor}80,
          0 4px 12px -4px ${theme.primaryColor}66,
      ...
      .call-view .end-call-btn:hover {
        background: linear-gradient(135deg, ${theme.accentColor} 0%, ${theme.primaryColor} 100%) !important;
        transform: scale(1.04);
      }
      ...
      .shivai-widget .shivai-fsp-send-btn {
        ...
        background: ${theme.primaryColor}; border: none; cursor: pointer;
        display: flex; align-items: center; justify-content: center;
  ...
  function closeWidget() {
    ...
    // Stop network polling when widget closes — prevents background health requests
...
// Converts AI responses (HTML or plain text with URLs) into clean formatted content.
  ...
  async function startConversation() {
    try {
      ...
      // widget_key comes from the widget config already fetched in
      // checkAgentStatusOnLoad() and stored on window.SHIVAI_WIDGET_CONFIG
      // (see agent-configs API response — widget.widget_key).
      ...
      // client-generated placeholder from before the request. end-call still
      // keys off room_name (unchanged); call_id is available on `data` if
      // ever needed elsewhere but is not used for end-call.
