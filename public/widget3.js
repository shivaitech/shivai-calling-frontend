(function () {
  ...
  (function () {
    ...
    if (!isWebsitePage) {
      _wlog('[widget3] Blocked: website widget only runs on public website pages.');
    ...
    try {
      ...
      if (window.SHIVAI_WIDGET_CONFIG && typeof window.SHIVAI_WIDGET_CONFIG === 'object') {
        ...
        if (widget.company_logo) {
          ...
          _wlog("??? Using company_logo from API widget config (S3 URL)");
        }

        if (widget.trigger_button_image) {
          triggerButtonImage = widget.trigger_button_image;
          _wlog("??? Using trigger_button_image from API widget config");
        }
      ...
      // ? PRIORITY 2: Check SHIVAI_CONFIG (legacy component state)
        ...
        if (config.content) {
          ...
          if (config.content.companyLogo) {
            ...
            _wlog("??? Using companyLogo from SHIVAI_CONFIG");
          }
          if (config.content.triggerButtonImage) {
            triggerButtonImage = config.content.triggerButtonImage;
            _wlog("??? Using triggerButtonImage from SHIVAI_CONFIG");
          }
        ...
        if (!triggerButtonImage && window.ShivAI && window.ShivAI.config && window.ShivAI.config.triggerButtonImage) {
          ...
          _wlog("??? Using triggerButtonImage from ShivAI.config");
        }
          ...
          if (script.src && script.src.includes('/widget2.js')) {
            try {
              ...
              if (urlCompanyLogo) {
                ...
                _wlog("??? Using companyLogo from URL parameter");
              }
              break;
            } catch (urlError) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (agentRes?.widget) {
          ...
          _wlog('?? No widget configuration found in agent response - getCompanyInfo() will use URL parameters or defaults');
        }
      } else {
  ...
  function makeDraggable(element) {
    const DRAG_THRESHOLD = 8; // px � below this we treat the gesture as a click
    let isDragging = false;
    let activePointerId = null;
    let startX, startY, initialX, initialY;

    element.style.cursor = "grab";
    element.style.touchAction = "none"; // let Pointer Events handle everything
  ...
  function makeWidgetDraggable(widgetElement) {
    ...
    let activePointerId = null;
    let startX, startY, initialX, initialY;
    // Track which header initiated the current drag so we can attach/detach
    // pointermove/pointerup on exactly that element (required for setPointerCapture)
    ...
    function startDrag(e) {
      if (e.pointerType === "mouse" && e.button !== 0) return;
      // Do not drag when the user clicks an interactive child element
      if (
        e.target.closest(
  ...
  function makeTriggerBtnDraggable(btnElement) {
    ...
    function drag(e) {
      ...
      // Reposition the message bubble relative to the button, keeping it in viewport
      if (messageBubble) {
        const bw = messageBubble.offsetWidth || 200;
        const bh = messageBubble.offsetHeight || 40;
        const btnH = btnRect.height;
        const btnW = btnRect.width;
        // Prefer left side; fall back to right side if it would clip
  ...
  // Position the widget panel so it always opens adjacent to the trigger button,
  // regardless of where the button was dragged.  Uses known CSS default dimensions
  // as fallbacks because the widget may still be display:none when this runs.
  function positionWidgetNearTrigger() {
    if (!triggerBtn || !widgetContainer) return;
    const btnRect = triggerBtn.getBoundingClientRect();
    // If the trigger hasn't been placed yet (getBoundingClientRect returns zeros)
    // just leave the CSS defaults in place.
  ...
  function createWidgetUI() {
    ...
    const bubbleCompanyInfo = getCompanyInfo();
    if (bubbleCompanyInfo.callToActionText) {
      liveMessages = [
        bubbleCompanyInfo.callToActionText,
        bubbleCompanyInfo.callToActionText,
        bubbleCompanyInfo.callToActionText,
        bubbleCompanyInfo.callToActionText,
      ];
  ...
  function openWidget() {
    // Recompute widget position based on current trigger location before showing
  ...
  function closeWidget() {
    ...
    hideConnectingState();
    if (ws) {
      _wlog("?? Closing WebSocket on widget close");
      ws.close();
      ws = null;
    }
  ...
  async function handleConnectClick(e) {
    ...
    if (!isConnecting && !isConnected && !isDisconnecting) {
      ...
      try {
        ...
        if (!isConnecting) {
          _wlog("?? Connection was cancelled during startup");
          return;
        }
  ...
  function addDocumentMessage(docData) {
    ...
    else if (typeStr.includes('website'))                       { iconEmoji = '🌐'; colorClass = 'link';   }
    else if (typeStr.includes('social'))                        { iconEmoji = '📱'; colorClass = 'social'; }
    else if (typeStr.includes('link'))                          { iconEmoji = '🔗'; colorClass = 'link';   }
    ...
    labelEl.textContent = 'AI Employee';

    const linkEl = document.createElement('a');
    linkEl.className = 'doc-card';
    linkEl.href      = url;
    linkEl.target    = '_blank';
    linkEl.rel       = 'noopener noreferrer';
    linkEl.title     = 'Click to open';
    linkEl.innerHTML = '<div class="doc-icon-wrap ' + colorClass + '">' + iconEmoji + '</div>'
      + '<div class="doc-meta"><div class="doc-name" title="' + name + '">' + name + '</div>'
      + secondaryInfo + '</div>' + actionBtn;
  ...
  async function startConversation() {
    try {
      ...
      if (!widgetKey) {
        console.warn("?? No widget_key found on SHIVAI_WIDGET_CONFIG — /widget-token call will likely be rejected");
      }
      ...
      room.on(
        ...
        (payload, participant, kind, topic) => {
          _wlog("?? DataReceived EVENT FIRED:", {
          ...
          try {
            ...
            if (text && text.trim().length > 0) {
              ...
              _wlog("?? Processing potential transcript:", text);

              // Try to parse as JSON first
              try {
                const jsonData = JSON.parse(text);
                _wlog("?? Parsed JSON data:", jsonData);
                ...
                if (Array.isArray(jsonData.documents) && jsonData.documents.length > 0) {
                  _wlog("📎 Documents array in transcript:", jsonData.documents.length, "item(s)");
                  setTimeout(function() {
                ...
                if (jsonData.type === "transcript" && jsonData.text) {
                  if (jsonData.role === "user") {
                    ...
                    if (!firstResponseReceived) {
                      _wlog("? First AI response received (legacy transcript)");
                      firstResponseReceived = true;
                ...
                _wlog("?? Not JSON, treating as plain text");
                ...
                if (text.length >= 2 && text.length <= 1000) {
                  ...
                  addMessage(senderRole, transcriptText);
                  _wlog("? Added plain text transcript:", {
                    role: senderRole,
                    text: transcriptText.substring(0, 50) + "...",
                  });
                }
