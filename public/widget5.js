(function () {
  ...
  const NETWORK_PING_INTERVAL = 30000; // 30 seconds — only when widget is open (connection API only, no HTTP ping)
  ...
  function isNovaAgentName(name) {
    return String(name || "").trim().toLowerCase() === "nova";
  }

  function updateNovaTriggerLayoutFlag(agentRes) {
    if (!agentRes) return;
    var names = [
      agentRes.name,
      agentRes.widget && agentRes.widget.ai_employee_name,
  ...
  function updateTriggerBasedOnStatus() {
    ...
    // Show the cloud bubble text (button_text) as the subtitle
    if (subtitleEl) {
      var info = getCompanyInfo();
      subtitleEl.textContent = info.callToActionText || (info.name ? "Talk to " + info.name : "AI Assistant");
    }
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (agentRes?.widget) {
          ...
          if (avatarSrc && !window.SHIVAI_WIDGET_CONFIG.trigger_button_image) {
            window.SHIVAI_WIDGET_CONFIG.trigger_button_image = avatarSrc;
            _wlog("🖼️ Promoted top-level agent avatar to trigger_button_image:", avatarSrc);
          }
          if (avatarSrc && !window.SHIVAI_WIDGET_CONFIG.company_logo) {
  ...
  /** Resolve theme colors from customization panel, API widget config, or defaults. */
  function getThemeColors() {
    var cfgTheme = (window.SHIVAI_CONFIG && window.SHIVAI_CONFIG.theme) || {};
    var api = window.SHIVAI_WIDGET_CONFIG || {};
    var infoTheme = getCompanyInfo().theme || {};

    return {
      primaryColor:
        cfgTheme.primaryColor ||
        api.primary_color ||
        infoTheme.primaryColor ||
        "#7c3aed",
      accentColor:
        cfgTheme.accentColor ||
        api.gradient_end ||
        infoTheme.accentColor ||
        "#6d28d9",
      secondaryColor:
        cfgTheme.secondaryColor ||
        api.gradient_start ||
        infoTheme.secondaryColor ||
        "#6d28d9",
    };
  }
  ...
  function buildThemedButtonOverrides(p1, p2) {
    ...
    var btnShadow =
      "0 8px 22px -8px " + p1 + "88, 0 4px 12px -4px " + p2 + "55, 0 1px 0 rgba(255,255,255,0.18) inset";
    return [
      ".start-call-btn { background: " + btnGrad + " !important; box-shadow: " + btnShadow + " !important; }",
      ".start-call-btn:hover { background: linear-gradient(135deg, " + p2 + " 0%, " + p1 + " 100%) !important; }",
      ".control-btn-icon.connect { background: " + p1 + " !important; box-shadow: 0 4px 22px " + p1 + "88 !important; }",
      ".control-btn-icon.connect:hover { background: " + p2 + " !important; }",
      ".shivai-trigger:hover:not(.dragging) .shivai-trigger-call-btn { background: " + p1 + " !important; color: #ffffff !important; box-shadow: 0 6px 18px -4px " + p1 + "88, 0 0 0 1px " + p1 + "55 !important; }",
  ...
  /** Glass bg only when API theme is original-default; all other themes use white. */
  function isGlassWidgetBackground() {
    var api = window.SHIVAI_WIDGET_CONFIG || {};
    if (api.theme) {
      return String(api.theme).toLowerCase() === "original-default";
    }
    // Customization preview fallback before API theme is available
    var widgetStyle =
      (window.SHIVAI_CONFIG && window.SHIVAI_CONFIG.widgetStyle) ||
      (window.SHIVAI_CONFIG && window.SHIVAI_CONFIG.theme && window.SHIVAI_CONFIG.theme.widgetStyle) ||
      api.widget_style ||
      "transparent-glass";
    return widgetStyle === "original" || widgetStyle === "transparent-glass";
  }
  ...
  function refreshWidgetTheme() {
    ...
    var p2 = colors.accentColor;
    var btnGrad = "linear-gradient(135deg, " + p1 + " 0%, " + p2 + " 100%)";
    var btnShadow = "0 8px 22px -8px " + p1 + "88, 0 4px 12px -4px " + p2 + "55, 0 1px 0 rgba(255,255,255,0.18) inset";
    var connectColor = p1;

    // Helper: reset widget container to base CSS (removes any inline overrides)
    function resetWidgetBg() {
      if (!widgetContainer) return;
      widgetContainer.style.background = "";
      widgetContainer.style.backdropFilter = "";
      widgetContainer.style["webkitBackdropFilter"] = "";
      widgetContainer.style.border = "";
      widgetContainer.style.boxShadow = "";
      var lv = widgetContainer.querySelector('.landing-view');
      if (lv) lv.style.background = "";
      var cv = widgetContainer.querySelector('.call-view');
      if (cv) cv.style.background = "";
    }

    if (isGlassWidgetBackground()) {
      // TRUE glassmorphism — only for API theme original-default
      if (widgetContainer) {
        widgetContainer.style.background = "rgba(225, 230, 238, 0.62)";
        widgetContainer.style.backdropFilter = "blur(40px) saturate(200%)";
        widgetContainer.style["webkitBackdropFilter"] = "blur(40px) saturate(200%)";
        widgetContainer.style.border = "1px solid rgba(255,255,255,0.55)";
      ...
      if (widgetContainer) {
        ...
        widgetContainer.style["webkitBackdropFilter"] = "none";
        widgetContainer.style.border = "1px solid rgba(0,0,0,0.08)";
        widgetContainer.style.boxShadow = "0 24px 48px -12px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.04)";
        var lvW = widgetContainer.querySelector('.landing-view');
        if (lvW) lvW.style.background = "#ffffff";
        var cvW = widgetContainer.querySelector('.call-view');
        if (cvW) cvW.style.background = "#ffffff";
      }
      // Inject button color overrides
      var overrideEl = document.createElement('style');
      overrideEl.id = 'shivai-theme-override';
      overrideEl.textContent = [
        ".start-call-btn { background: " + btnGrad + " !important; box-shadow: " + btnShadow + " !important; }",
        ".start-call-btn:hover { background: linear-gradient(135deg, " + p2 + " 0%, " + p1 + " 100%) !important; }",
        ".control-btn-icon.connect { background: " + connectColor + " !important; box-shadow: 0 4px 22px " + p1 + "88 !important; }",
        ".control-btn-icon.connect:hover { background: " + p2 + " !important; }",
        ".shivai-trigger:hover:not(.dragging) .shivai-trigger-call-btn { background: " + p1 + " !important; color: #ffffff !important; box-shadow: 0 6px 18px -4px " + p1 + "88, 0 0 0 1px " + p1 + "55 !important; }",
        ".landing-headline { color: #1a1a2e !important; }",
        ".widget-title, .landing-agent-name { color: #1a1a2e !important; }",
  ...
  function refreshWidgetContent() {
    ...
    if (widgetAvatar && logoSrc) {
      widgetAvatar.innerHTML = `<img src="${logoSrc}" alt="${companyInfo.name} Logo" style="${imgStyle}">`;
      _wlog("✅ Updated landing view logo to:", logoSrc);
    }
    ...
    if (callAvatar && logoSrc) {
      callAvatar.innerHTML = `<img src="${logoSrc}" alt="${companyInfo.name} Logo" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block;">`;
      _wlog("✅ Updated call view avatar");
    }

    // Update landing card avatar
    const landingAgentAvatar = widgetContainer && widgetContainer.querySelector('.landing-agent-avatar');
    if (landingAgentAvatar && logoSrc) {
      landingAgentAvatar.innerHTML = `<img src="${logoSrc}" alt="${companyInfo.name}" style="width:100%;height:100%;object-fit:cover;display:block;">`;
    ...
    // Keep animated cloud bubble messages in sync
  ...
  window.ShivAIWidget.open = openWidget;
  ...
  function createWidgetUI() {
    ...
    const avatarMarkup = triggerAvatarSrc
      ? `<img class="shivai-trigger-avatar" src="${triggerAvatarSrc}" alt="${triggerName}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block;max-width:none;">`
      ...
      <div class="landing-content">
        ...
        <div class="landing-agent-card">
          <div class="landing-agent-avatar">
            ${companyInfo.logo
              ? `<img src="${companyInfo.logo}" alt="${companyInfo.name}" style="width:100%;height:100%;object-fit:cover;display:block;max-width:none;">`
  ...
  function setDefaultLanguage() {
    ...
    for (let i = scriptTags.length - 1; i >= 0; i--) {
      ...
      if (s.src && (s.src.includes('/widget2.js') || s.src.includes('/widget.js') || s.src.includes('/widget3.js') || s.src.includes('/widget4.js') || s.src.includes('/widget5.js'))) {
        try {
  ...
  function getEffectiveLatencyMs() {
    ...
    // Use navigator.connection.rtt — accurate, no HTTP request needed
  ...
  function measureAndUpdateNetwork() {
    // Use only the browser's Network Information API — no HTTP requests, no CORP issues.
    // navigator.connection.rtt is an accurate latency estimate provided by the browser.
    readConnectionApi();
    updateNetworkIndicator();
  }
  ...
  function startNetworkMonitor() {
    ...
    // Initial ping only when widget is first opened (called from openWidget)
    measureAndUpdateNetwork();
    if (networkMonitorTimer) clearInterval(networkMonitorTimer);
    // Poll every 30s while widget is open; stopNetworkMonitor() clears this on close
  ...
  function addWidgetStyles() {
    ...
    const styles = `
      ...
      .shivai-trigger--bottom-center:active:not(.dragging) {
      ...
      .shivai-trigger--bottom-center { left:50% !important; right:auto !important; bottom:calc(14px + env(safe-area-inset-bottom)) !important; transform:translateX(-50%); }
  ...
  function closeWidget() {
    ...
    // Stop network ping polling when widget closes — prevents background health requests
...
// Converts AI responses (HTML or plain text with URLs) into clean formatted content.
  ...
  // One delegated click/keyboard listener on the messages container handles
  // every .shivai-phone-chip, including ones added after this runs.
  ...
  function openPhoneChipPopover(chipEl, phoneDigits) {
    ...
    pop.innerHTML =
      ...
      + '<a class="shivai-phone-popover-item" data-action="whatsapp" href="https://wa.me/' + phoneDigits.replace(/^\+/, '') + '" target="_blank" rel="noopener noreferrer">'
        + '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12.001 2C6.478 2 2 6.477 2 12c0 1.821.487 3.53 1.338 5.003L2 22l5.135-1.315A9.947 9.947 0 0 0 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 18.056a8.014 8.014 0 0 1-4.086-1.117l-.293-.175-3.048.78.813-2.973-.19-.306A8.02 8.02 0 1 1 20.02 12a8.034 8.034 0 0 1-8.02 8.056z"/></svg>'
        + '<span>WhatsApp</span>'
      + '</a>';
    document.body.appendChild(pop);
    // Position after appending so we can measure its own size, clamped inside the viewport.
